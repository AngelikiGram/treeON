...
from dataset_colors import TreeDataset
...
parser.add_argument('--top_k_shadows', type=int, default=1000)
...
def compute_losses_and_forward_pass(network, data, light_directions, opt, lpips_loss_fn, device, is_training=True):
    """
 ...
 #   orthophoto = (orthophoto - 0.5) / 0.5
    ...
    if opt.colors:
        ...
        color_loss = F.mse_loss(pred_colors, final_colors) * 100.0 # * 10000
        print('Color loss:', color_loss.item())

    if opt.classes_loss:
        # Compute class loss
        class_loss = F.cross_entropy(class_logits, labels)
        loss_class = class_loss * 100.0
    ...
    if opt.shadow:
        ...
        shadow_occ_loss = lpips_loss_fn(pred_shadow_img2, gt_shadow_img2).mean() #* 5
    ...
    if opt.silhouettes:
        ...
        silh_loss = silhouette_loss['total_loss']
...
if __name__ == "__main__":
    ...
    # Load Dataset
    # ===========================
    dataset = TreeDataset(npoints=opt.num_points, num_trees=opt.num_trees)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=opt.batchSize, num_workers=int(opt.workers), shuffle=True, collate_fn=collate_fn)

    dataset_test = TreeDataset(npoints=opt.num_points, train=False, num_trees=opt.num_trees)
    dataloader_test = torch.utils.data.DataLoader(dataset_test, batch_size=opt.batchSize, num_workers=int(opt.workers), shuffle=False, collate_fn=collate_fn)

    # ===========================
    # Load Model
    # ===========================
    network = TreeReconstructionNet(num_points=opt.num_points)
    network.cuda()
    ...
    with open("/home/grammatikakis1/TREES_DATASET/ORTHOPHOTOS/light_directions.txt", "r") as f:
    ...
    for epoch in range(start_epoch, opt.nepoch):
        ...
        for i, data in enumerate(dataloader):
            result = compute_losses_and_forward_pass(network, data, light_directions, opt, lpips_loss_fn, device, is_training=True)
            ...
            print(f"[Epoch {epoch+1}/{opt.nepoch}] Batch {i+1}/{len(dataloader)} Loss: {result['loss'].item():.4f}  -- Tree Loss: {result['tree_occ_loss'].item():.4f} -- Shadow Loss: {result['shadow_occ_loss'].item():.4f} -- Silh Loss: {result['silh_occ_loss'].item():.4f} -- Color Loss: {result['color_loss'].item():.4f} -- lr: {optimizer.param_groups[0]['lr']}")
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(dataloader_test):
                result = compute_losses_and_forward_pass(network, data, light_directions, opt, lpips_loss_fn, device, is_training=False)
                last_result = result  # Store for loss tracking
                
                val_loss.update(result['loss'].item())
                
                print(f"[Validation] Epoch {epoch+1}/{opt.nepoch} Batch {i+1}/{len(dataloader_test)} Loss: {result['loss'].item():.4f} -- Tree Loss: {result['tree_occ_loss'].item():.4f} -- Shadow Loss: {result['shadow_occ_loss'].item():.4f} -- Silh Loss: {result['silh_occ_loss'].item():.4f} -- Color Loss: {result['color_loss'].item():.4f}")
                ...
                if i == len(dataloader_test) - 1:
                    ...
                    # Regular visualization
                    visualize(network, vis, "Val", result['dsm_pc'], result['query_points'], gt_points_vis, occupied_points_vis, result['orthophoto'], result['gt_mesh'], result['gt_occupancy'], result['occupancy_pred'])

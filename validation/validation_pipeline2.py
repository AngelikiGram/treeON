...
# def print_evaluation_results(results):
#     """Print evaluation results in an organized format."""
#     print("\n" + "="*80)
#     print("🌲 TREE RECONSTRUCTION EVALUATION RESULTS 🌲")
#     print("="*80)
...
#     # Basic Statistics
#     print(f"\n📊 DATASET STATISTICS:")
#     print(f"{'Total Samples Processed:':<30} {results['total_samples']}")
#     print(f"{'Query Points per Tree:':<30} {results['num_query_points']}")
#     print(f"{'Output Points per Tree:':<30} {results['num_points']}")
    
#     # Geometric Accuracy Metrics
#     print(f"\n🎯 GEOMETRIC ACCURACY METRICS:")
#     print(f"{'Chamfer Distance (RMSE):':<30} {results['chamfer_rmse']:.4f} meters")
#     print(f"{'Normalized Chamfer Distance:':<30} {results['normalized_chamfer']:.4f}")
#     print(f"{'Chamfer Std Dev:':<30} {results['chamfer_std']:.4f} meters")
    
#     # Completeness and Precision Metrics
#     print(f"\n🎪 COMPLETENESS & PRECISION METRICS:")
#     print(f"{'F1 Score:':<30} {results['f1_score']:.4f}")
#     print(f"{'Precision:':<30} {results['precision']:.4f}")
#     print(f"{'Recall:':<30} {results['recall']:.4f}")
    
#     # Generative Quality Metrics
#     print(f"\n🔄 GENERATIVE QUALITY METRICS:")
#     print(f"{'Coverage:':<30} {results['coverage']:.4f}")
#     print(f"{'MMD (lower is better):':<30} {results['mmd']:.4f}")
#     print(f"{'COV (diversity):':<30} {results['cov']:.4f}")
    
#     # Inter-sample Analysis
#     print(f"\n🔍 INTER-SAMPLE ANALYSIS:")
#     print(f"{'Unique GT Matches:':<30} {results['unique_matches']}/{results['total_predictions']}")
#     print(f"{'Variance Score:':<30} {results['variance_score']:.2f}%")
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for batch_idx, (orthophoto, gt_mesh, dsm_points, index_views, filenames, center_dsm, scale_dsm) in enumerate(dataloader):
            ...
            # Generate query points based on variable setting
            if args.variable == '0':
                query_points = create_adaptive_query_points(B, dsm_points, num_query_points=args.num_query_points, near_tree_ratio=0.9)
            elif args.variable == '1':
                ...
                query_points = sample_points_in_convex_hull(B, dsm_points, num_query_points=args.num_query_points).to(device)

            # Compute ground truth occupancy
            # gt_occupancy = compute_occupancy(B, query_points, gt_mesh, top_k=args.top_k, threshold=25)
        ...
        for p in range(len(all_pred_points)):
            ...
            # print('size of gt_pts:', gt_pts.shape, 'size of pred_pts:', pred_pts.shape, 'size of dsm_pts:', dsm_pts.shape)

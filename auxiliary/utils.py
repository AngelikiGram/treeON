...
def weights_init(m):
    ...
    - Uses Kaiming Normal for Conv1d layers (better for ReLU/LeakyReLU).
    - Uses Xavier Uniform for Linear layers (better for stable training).
    - Applies small variance normal initialization for BatchNorm.
    """
    if isinstance(m, nn.Conv1d):
        nn.init.kaiming_normal_(m.weight, nonlinearity='leaky_relu')  # Better for Conv layers
        if m.bias is not None:
            nn.init.constant_(m.bias, 0)
    
    elif isinstance(m, nn.Linear):
        nn.init.xavier_uniform_(m.weight)  # Good for fully connected layers
        if m.bias is not None:
            nn.init.constant_(m.bias, 0)

    elif isinstance(m, nn.BatchNorm1d):
        nn.init.normal_(m.weight, mean=1.0, std=0.01)  # Keep variance controlled
        nn.init.constant_(m.bias, 0)
...
def get_boundary(faces):
    ...
    unique_index = np.unique(edges_unrolled, return_index=True)[1]
    repeated_index = np.array(list(set(all_index).difference(set(unique_index))))
    repeated_index = torch.tensor(repeated_index, dtype=torch.long, device='cuda')  # Convert to PyTorch tensor
    unique_value = edges_unrolled[unique_index]
    repeated_value = edges_unrolled[repeated_index.cpu().numpy()]
...
def prune(faces_cuda_bn, error, tau, index, pool='max', faces_number=5120):
    ...
    if face_error.shape[1] != faces_cuda_bn.shape[1]: ##
        # Ensure `face_error` is mapped to the correct number of faces
        face_error = face_error[:, :faces_cuda_bn.shape[1]]

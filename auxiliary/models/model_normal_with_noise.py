...
class ImplicitOccupancyDecoder(nn.Module):
    ...
    def forward(self, latent, query_points):
        ...
        latent_expanded = latent.unsqueeze(1).expand(-1, N, -1)  # [B, N, 2112]
        x = torch.cat([latent_expanded, encoded_points], dim=-1)  # [B, N, 2265]
        x = x.view(B * N, -1) 
...
class TreeReconstructionNet(nn.Module):
    ...
    def forward(self, dsm_pc, orthophoto, query_points, training=False, light_dir=None):
        ...
        if training:
            combined_latent += 0.05 * torch.randn_like(combined_latent)

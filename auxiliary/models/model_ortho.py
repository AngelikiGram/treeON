...
class TreeReconstructionNet(nn.Module):
    ...
    def forward(self, dsm_pc, orthophoto, query_points, light_dir=None):
        ...
        class_logits = self.classifier(latent_img) # combined_latent)
        category_embed = self.category_predictor(latent_img) # combined_latent)
        latent = torch.cat([latent_img, category_embed], dim=1)

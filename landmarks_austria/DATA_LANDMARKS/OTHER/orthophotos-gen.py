...
OBJ_FOLDER = "F:/conifers_simplified"  # Path to OBJ tree files
DTM_FOLDER = "DATA-GEN//ORTHOPHOTOS//dtms"  # Path to DTM (terrain) files
DTM_FOLDER = os.path.join(base_folder, DTM_FOLDER)
ORTHO_FOLDER = "DATA-GEN\ORTHOPHOTOS//orthophotos"  # Path to orthophoto files
ORTHO_FOLDER = os.path.join(base_folder, ORTHO_FOLDER)
OUTPUT_FOLDER = "F://conifers_orthophotos"  # Output for shadow images
...
texture_path = 'DATA-GEN//ORTHOPHOTOS//textures/coniferous.jpg'
...
def render_shadow_views(camera, sun, output_dir, ground_plane, tree):
    ...
    for i in range(views_40):
        ...
        save_render(i, output_dir, camera)
...
def save_render(index, output_dir, camera):
    # ** 
...
def main(): # start, end):
    ...
    for tree_file in tree_files: # [:7000]:
        ...
        render_shadow_views(camera, sun, output_dir, dtm, tree) # output_dir_shadows

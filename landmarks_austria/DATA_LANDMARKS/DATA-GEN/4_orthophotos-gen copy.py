...
OBJ_FOLDER = "F:/TREES/TREES-temp" # NEW/TREES_OBJ" # TREES/TREES-temp" # NEW/TREES_OBJ" # TREES-temp" # deciduous" # conifers"  # Path to OBJ tree files
...
def setup_side_camera(tree, dtm, view_angle_deg=0, distance=10, elevation_above=1):
    center = get_object_center(tree)

    # Camera position on a horizontal circle
    theta = math.radians(view_angle_deg)
    x = center.x + distance * math.cos(theta)
    y = center.y + distance * math.sin(theta)

    # Get elevation at tree center from DTM
    elevation_dtm = get_dtm_height_at_point(dtm, x, y)
    if elevation_dtm is None:
        print(f"Skipping view at angle {view_angle_deg} — no DTM height available.")
        return None

    z = elevation_dtm + elevation_above  # camera height
    ...
    bpy.context.scene.camera = camera
    return camera
def save_camera_pose(camera, tree, save_path):
    """
    Saves the camera's 4x4 transformation matrix to file.
    If relative_to_object is provided (e.g. the tree), saves camera pose in that object's coordinate frame.
    """

    print("Blender location:", camera.location)

    # Get camera world matrix
    cam_matrix_world = camera.matrix_world

    if tree is not None:
        # Get inverse of the object's world matrix to move to local frame
        object_to_world = tree.matrix_world
        world_to_object = object_to_world.inverted()
        # Transform camera into object's coordinate frame
        cam_matrix = world_to_object @ cam_matrix_world
    else:
        cam_matrix = cam_matrix_world

    # Save as flat 4x4 matrix (row-major)
    with open(save_path, 'w') as f:
        for row in cam_matrix:
            f.write(' '.join(f'{v:.6f}' for v in row))
            f.write('\n')
def save_camera_pose1(camera, tree, save_path):
    """
    Saves the camera pose in tree-centered coordinates (tree at origin).
    """
    # Tree world matrix (transform from local → world)
    tree_to_world = tree.matrix_world

    # Invert to get world → tree transform
    world_to_tree = tree_to_world.inverted()

    # Camera world matrix
    cam_world = camera.matrix_world

    # Compute camera matrix in tree coordinates:
    # transforms from tree space → camera space
    tree_to_cam = world_to_tree @ cam_world

    # Save inverse of tree_to_cam: world-to-camera from tree-centered frame
    cam_pose = tree_to_cam.inverted()  # camera-to-tree, inverted for correct projection
...
def render_side_views(tree, dtm, output_dir, num_views=8, distance=10):
    ...
    for i in range(num_views):
        ...
        camera = setup_side_camera(tree, dtm, view_angle_deg=angle, distance=distance)

        if camera is None:
            print(f"⚠️ Skipping side view {i} due to camera setup failure.")
            continue
        ...
        if dtm_name in bpy.data.objects:
            dtm = bpy.data.objects[dtm_name]
            dtm.hide_render = True  # Hide from final render
            dtm.hide_viewport = False  # Optional: still visible in viewport
        ...
        bpy.context.scene.render.filepath = file_path
        bpy.ops.render.render(write_still=True)
        save_camera_pose(camera, tree, pose_path)
        
        print(f"📸 Saved side view {i} and pose.")
